...
namespace Kiwi
{
	...
	LRESULT RenderWindow::HandleMessage(UINT msg, WPARAM wParam, LPARAM lParam)
	{
		...
		switch( msg )
		{
			case WM_INPUT:
				{
					m_inputDevice.ProcessInput( wParam, lParam );
					break;
				}
		}

		//send this event to the listeners
		this->BroadcastWindowEvent(Kiwi::WindowEvent(this, msg));
	...
	void RenderWindow::Update( float deltaTime )
	{
		...
		m_inputDevice.OnUpdate( deltaTime );
	...
	/*void RenderWindow::BroadcastMouseEvent( const Kiwi::MouseEvent& evt )
	{
		if( m_mouseFocus != 0 )
		{
			switch( evt.GetEventType() )
			{
				case MouseEvent::MOUSE_HELD:
					{
						m_mouseFocus->OnMouseHeld( evt );
						break;
					}
				case MouseEvent::MOUSE_MOVE:
					{
						m_mouseFocus->OnMouseMove( evt );
						break;
					}
				case MouseEvent::MOUSE_PRESS:
					{
						m_mouseFocus->OnMousePress( evt );
						break;
					}
				case MouseEvent::MOUSE_RELEASE:
					{
						m_mouseFocus->OnMouseRelease( evt );
						break;
					}
				default: return;
			}
