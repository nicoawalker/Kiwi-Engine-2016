...
namespace Kiwi
{
	...
	DepthStencil::DepthStencil( std::wstring name, Kiwi::Renderer* renderer, const Kiwi::Vector2& dimensions, unsigned int mipLevels, unsigned int arraySize )
	{
		...
		//also create the depth-disabled state
		...
		hr = renderer->GetDevice()->CreateDepthStencilState( &depthDisabledDesc, &m_depthDisabledState );
		if( FAILED( hr ) )
		{
			throw Kiwi::Exception( L"DepthStencil", L"[" + m_name + L"] Failed to create depth disabled stencil state" );
		}
