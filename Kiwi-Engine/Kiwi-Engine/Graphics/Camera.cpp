...
namespace Kiwi
{
	...
	void Camera::_CalculateViewMatrix()
	{
		...
		Kiwi::Vector3d front = m_transform->GetForward();
		Kiwi::Vector3d up = m_transform->GetUp();
		Kiwi::Vector3d right = m_transform->GetRight();

		Kiwi::Vector3d pos = m_transform->GetPosition();
